...
3. Set up environment variables:
   ...
   ```
   # Tenant store (Upstash Redis)
   KV_REST_API_URL=your_redis_url
   KV_REST_API_TOKEN=your_redis_token

   # Root domain for subdomain routing (e.g. yourbrand.com)
   NEXT_PUBLIC_ROOT_DOMAIN=localhost:3000

   # Lead delivery (Resend). Optional — the contact form degrades
   # gracefully and logs leads if these are unset.
   RESEND_API_KEY=your_resend_key
   LEAD_FROM_EMAIL=Leads <onboarding@resend.dev>
   DEFAULT_LEAD_INBOX=you@yourbrand.com
   ```
